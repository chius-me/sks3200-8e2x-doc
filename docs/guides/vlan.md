...
本次未验证：把某口设成某 VLAN 的 Untagged 成员时，PVID 会不会自动改成该 VLAN。实际配置时两页都要再看一遍。

## 和常见 Access / Trunk 的对照

界面没有这些词。下面只是用两页字段去**近似**常见意图。完整隔离还取决于其它 VLAN 的成员关系。当前 UI 未展示独立的入口成员过滤开关，因此不能只改「仅无标签」或只改 PVID 就宣称严格隔离。此处也未见 VLAN 间路由。
